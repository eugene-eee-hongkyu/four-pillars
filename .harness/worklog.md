...
## Session 2026-09-18 15:59 — 리포트 배포 흐름 전환 + 약관·결제 정합 마이그레이션

### 작업 요약
- 리포트 제공 방식 전환: 이메일 첨부 → 보관소 링크 + 1년 다운로드 제공 (직접 제공 위험 회피 위해 2단계 방식 도입)
- 약관·환불정책 정합 구현: 결제 전 동의 내용 서버 기록 필수화
- 환불 분쟁 대비 3종 정책(전액환불·부분환불·환불불가) 마련
- 회원가입 시점에 약관·개인정보처리방침 동의 기록
- 마이그레이션 3건 적용 (코드 커밋 9건 + 기록 커밋 1건 모두 main 반영)

### 실패한 시도
- worklog 기록 오류: "4건" → 실제는 "3건"으로 정정 후 재커밋

### 다음 액션
- 약관 잔여 정비 (Resend 위탁, PDF 2년 보관, 국외 이전 고지, 회원탈퇴 방법, 제공 시점, 면책 단서 등 8개 항목)
- 토스 검수 재진행 + 실판매 시 라이브 키 교체
- 어드민 '1년 연장' 버튼 검증

---

파일로 저장할까요? (`/Users/eugene/.claude/projects/-/memory/session_2026_09_18.md`)


## Session 2026-09-18 14:52 — 리포트 제공을 '이메일 첨부 → 보관소 + 1년 다운로드 링크'로 전환 + 약관·환불정책 정합 + 환불 분쟁 대비 3종

### 작업 요약
- **배경**: 결제(PG) 측에서 상품 '이용기간 1년' 명시 요구 → PDF를 메일에 첨부(영구)하던 방식을 링크 제공·1년 만료로 전환. 서비스 개시 전이라 약관·문구도 즉시 반영
- **보관소 + 다운로드 API** (`ad23bd3`): Supabase Storage 비공개 버킷 `report-pdfs`(`orders/<orderId>/summary|detail.pdf`). payment_orders에 summary/detail_pdf_path·download_token(48hex)·download_expires_at(결제일+1년)·first_downloaded_at·last_resend_at. `lib/payments/report-storage.ts` 신설. `/api/download` 신설
- **발송 전환** (`d61dc7a`): fulfill/fulfill-detail이 PDF 생성 → 보관소 저장 → 링크 메일(만료일 표기, 첨부 제거). **보관 파일이 있으면 재생성 없이 링크만 재발송**(상세 14섹션 재생성 비용 0). 예전 주문은 재발송 시 자동으로 생성·보관(하위호환). `/api/reports`: 재발송 3회 제한 폐지 → 주문당 60초 간격만(Resend 일일 한도 보호), 만료 주문 재발송 410, GET에 만료일·바로 다운로드 링크. `/api/admin/payments`: `extendDays`로 이용기간 연장(기준 max(지금, 현재 만료)+N일, 감사 로그 `extend_download_expiry`). 크론 `purge-expired-pdfs`(매일 03시 KST): 만료+1년(기본 결제일+2년) 지난 파일 삭제, 주문 기록은 보존
- **UI·문구** (`f29a869`): `/reports`에 바로 내려받기·만료일·만료 시 버튼 숨김(횟수 제한·확인 팝업 제거). `/admin/payments`에 만료일·최초 다운로드·보관 파일 상태 + '1년 연장'. 상품 설명 '영구 소장' → '링크 제공·결제일로부터 1년'. 결제 완료·결제 화면 안내 문구 정리
- **약관·환불정책** (`7442bce`·`3ab0021`·`c338f20`): 약관 제6조 4-6항(링크 제공·1년·만료는 환불 사유 아님) 신설, 제4조를 실제 범위로 정정(비회원 1명·7영역 / 회원 5명·14영역 / 유료=14영역 요약+상세. 기존 '20영역·영역 5개' 오기), **제6조 1항 가격 20,000원 → 실제 판매가 30,000원(PDF_REPORT.price) 불일치 수정**, 제2조 4항 정의. 환불정책: 사업자 귀책 사유를 링크 모델로 재작성(+이용기간 내 회사 문제로 다운로드 불가 시 100% 환불), '다운로드 이용기간(1년)과 환불' 조항 신설, 번호 5-8 재정렬. 시행일 2026-09-18
- **환불 분쟁 대비 3종** (`c338f20`·`7483715`): (1) 결제 화면 필수 동의 체크(내려받은 뒤 환불 제한 + 이용기간 1년, 무료 진단=사전 체험 안내). 서버 `/api/payments/order`도 동의 없으면 400, `refund_policy_agreed_at`에 동의 시각 기록. (2) `/api/download` 2단계화 — GET=확인 페이지(기록 없음), POST=실제 다운로드+최초 시각 기록. 메일 보안 프로그램의 자동 링크 방문으로 '다운로드함'이 잘못 기록되는 문제 차단. 최초 다운로드 전 고객에겐 확인 페이지에서 환불 제한 재고지. (3) 환급 기한 7영업일 → 3영업일(전자상거래법 제18조). 동의 체크박스 aria-checked 보강
- **CRON_SECRET 등록**: Vercel production env에 무작위 64hex 등록(값은 대화·로그·파일에 남기지 않음). 무인증 호출 401, Vercel 자동 스케줄 실행은 200(14:35 tick 로그로 확인)
- **마이그레이션 3건 적용**: `20260918000000_report_pdf_storage` · `20260918010000_download_expiry_admin_and_resend_cooldown` · `20260918020000_payment_orders_refund_consent` (+ 08-13 resend_count는 이제 미사용, 컬럼은 무해하여 유지)
- **프로덕션 검증**(운영자 테스트 주문 order_0141…): 재발송 → 실제 PDF 보관(요약 146KB·상세 304KB) + 링크 메일 / 다운로드 200·한글 파일명 정상 / 60초 연타 429 / 잘못된 토큰 404 / 만료 → 안내 페이지 410·재발송 410·구매 내역 링크 숨김 / 만료일 복원 → 재개 / 2년 경과 조건에서 크론이 파일 2개만 삭제·주문 보존(실행 전 대상이 테스트 주문 1건뿐임을 SQL로 확인) / GET만으로는 first_downloaded_at 미기록, POST로 기록 / 동의 없는 주문 400, 동의 주문에 시각 기록(테스트용 pending 1건 삭제). 테스트 주문은 원래 만료일(2027-08-13)로 복구, 파일 재생성해 둠. 운영자 주소로 링크 메일 4통 발송됨

### 실패한 시도
- **Storage 서명 URL의 `download` 파일명** → 한글이 이중 인코딩되어 저장 시 `%ED%85…pdf`로 깨짐. 다운로드 API가 파일을 직접 전달(Content-Disposition filename* RFC 5987)하도록 전환. 4MB 초과 시에만 서명 URL(영문 파일명) 우회
- **Bash heredoc으로 TS 파일 작성** → 셸 훅이 본문 속 논리 연산자를 복합 명령으로 오인해 차단. 코드 파일은 Write/Edit 도구로(메모리 기록)
- **배포 대기 스크립트** → `vercel ls`가 상태 표를 stderr로 내보내 `2>/dev/null`에선 Ready 판정 불가. `2>&1`로 수정
- 약관 가격 옆 '(부가세 포함)' 추가 → 세금 처리 방식 미확인 사실이라 즉시 제거

### 다음 액션
- 남은 약관 정비(문구): 개인정보처리방침(Resend 위탁 누락·PDF 최대 2년 보관·결제 시 수집항목=이메일만·구글 로그인·국외 이전 고지), 회원 탈퇴 방법 명시, 제공 시점(요약 즉시·상세 당일) 명시, 제7조 전면 면책에 '고의·중과실 제외' 단서 → backlog 등록
- 어드민 '1년 연장' 버튼 클릭 검증(어드민 계정 필요). API 로직은 타입체크 통과, 다운로드 측 재개는 DB로 검증됨
- 운영 주의: 환불 요청은 3영업일 내 토스 결제 취소. 환불 가능 여부는 어드민 결제 화면의 '아직 다운로드 안 함'/최초 다운로드 날짜로 판단

## Session 2026-08-15 14:59 — 워크로그 기록 및 상태 갱신

### 작업 요약
- worklog.md 2026-08-14 항목을 "검증 완료"로 갱신
- state.md 전체 재작성 ("검증 완료" 상태로 변경)
- 파일 크기·설정·git 상태 확인 (모두 정상)
- worklog.md·state.md·backlog.md·decision.md 커밋 및 푸시
- 프로젝트 알림 일시 중지 (snooze_attention)

### 다음 액션
- 토스 검수자에게 비회원 결제 도달 경로 안내 후 검수 재진행
- 실판매 개시 시 토스 라이브 키로 교체 (현재 테스트 키)
- Anthropic API 정식 키로 교체 권장 (현재 구독 OAuth 토큰)


## Session 2026-08-14 14:59 — 비회원 결제 진입로 추가 (토스 검수 대응) + 프로덕션 검증 완료

### 작업 요약
- 토스 PG 검수 요구사항 분석: 검수자가 로그인 없이 결제까지 도달 가능해야 함
- **비회원 결제 막힘 원인 파악**: Part1은 무료지만, Part2 진입에서 카카오 로그인 강제 → 결제 버튼은 Part2 완료 후에만 노출 → 비회원은 결제 버튼 자체를 못 봄. checkout 화면 자체는 로그인·Part2 불필요(sessionId+childSubjectId만 필요)임을 코드로 확인
- **A 방안 구현** (`48fe16b`): interpret-premium.tsx Part1 완료 직후 블록에 `!user` 조건으로 결제 CTA 추가(이미 결제한 세션이면 '구매 내역 보기'). 회원 퍼널(Part2 로그인 유도)은 그대로. 타입체크 0 + 웹 빌드 통과, git push main 배포
- **프로덕션 검증**(luck.z21labs.world, 로그인 없는 신규 세션 주입): 주의 — premiumPart1Text만 주입하면 `premiumPromptVersion` mismatch로 무효화되어 StreamingBody가 대신 실행(가짜 세션 404). `premiumPromptVersion: 'v6.0-14sections-merge'` 함께 주입해야 part1Done 유지됨. 결과: Part1 완료 화면에 로그인 버튼 + '📄 정밀 학운 리포트 PDF로 받기(30,000원)' 동시 노출 → 클릭 시 `/checkout` 이동(로그인 요구 없음) → 토스 결제위젯 정상 렌더(테스트 환경 안내 + 결제수단 + 약관 + 결제하기). 실결제는 안 함(검증용 가짜 세션)

### 다음 액션
- 실제 판매 개시 시 토스 라이브 키로 교체(현재 테스트 키 — 결제창에 '실제로 결제되지 않아요' 표기)
