...
## Session 2026-05-18 13:50 — 만세력 정확도 개선 배포 전략 결정

### 작업 요약
- A안(양쪽 즉시 적용)과 B안(학운 선행)의 트레이드오프 검토
- 사주톡 진행 중인 테스트 vs 정확도 향상 사이 선택지 분석
...
**이유**:
1. **사주톡 테스트 데이터 신뢰성**: 10명 피드백은 귀한 자산 — 변수 추가하면 결과 해석 복잡해짐
2. **비용 vs 리턴**: 테스트 남은 기간 짧으면 A안 전환 비용이 얻는 이득보다 큼  
3. **학운으로 검증 먼저**: 신규 프로젝트라 만세력 정확도 문제 먼저 포착 가능 → 사주톡 마이그레이션 때 근거 생김
4. **2차 배포 명분**: "학운 유저 피드백 반영했으니 안정적" → 사주톡 사용자 설득 용이

**A안이 나을 경우**: 테스트 종료일이 임박했다면 (< 1주일), A안으로 정확한 기반부터 시작하는 게 나음.
