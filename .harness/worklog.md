...
## Session 2026-05-19 20:49 — 학운 알고리즘 결정성 계산화 + calibration + confidence 구간 + 외부 학술 검증

### 작업 요약

**계산 영역 vs 해석 영역 재정립**
- 사용자 진단: "명리에서 명확한 계산 영역인데 LLM 해석에 맡긴 부분 있음" → 알고리즘이 매번 흔들리는 원인
- audit 후 코드 결정성 계산으로 옮긴 영역: 학운 단계, 베이스 티어, 부모 환경 조정, 격국 진로 매핑, 현재 학운 시기, 신왕신약, 청소년기 대운, confidence 구간

**hagun-tier.ts 알고리즘 11종 보강 (단일 세션에 누적)**
- 재극인(財剋印) 페널티 -2 — 재성 ≥3 AND 재성 ≥ 2×(인성+1) AND 4귀인 ≤1
- 4귀인 0회 페널티 -1 — 학문 친화 시그널 부재
- 비학문 강세 일반화 -2 — (식상+재성+비겁 ≥4) AND (인성+관성 ≤2) AND 4귀인 ≤1 AND 학자형 격국 ✗
- 자립 학자형 +3 — 정인격·편인격·정관격 + 일지 건록·제왕 + 비겁 ≥2 (POSTECH 1티어 패턴)
- 학자형 시그널 부재 콤보 -2 — 4귀인 0 + 학자형 격국 ✗ + 일지 weak
- 12운성 '쇠' WEAK 분류 (이전 누락 버그 fix)
- 합·삼합 +2 → +1 (너무 흔하게 발동되어 약화)
- 일지 12운성 강 +1 → +2 (학습 자리 핵심)
- 학자형 격국 +2 (정관·정인·편인·식신·건록격)
- 신왕신약 통합 평가 ±1 — (비겁+인성) - (식상+재성+관성/2) + 월·일지 12운성
- 청소년기 대운 ±2 — 6~22세 대운 stemSipsin·branchSipsin 인성·관성 +1 / 재성 -1
- 점수 cutoff 재조정: ≥10 매우강 / ≥7 강 / ≥4 중상 / ≥2 중 / ≥0 중하 / ≥-2 약상 / ≥-4 약중 / ≥-6 약하 / ≤-7 매우약

**Confidence 구간 도입** ([2d58e71](https://github.com/eugene-eee-hongkyu/four-pillars/commit/2d58e71), [ea7819b](https://github.com/eugene-eee-hongkyu/four-pillars/commit/ea7819b))
- 같은 단계 안에서도 점수에 따라 certain/likely/reach 3분류
- 한국 입시 컨설팅 표준 (도전/적정/안정) 1:1 매핑
- ≥15 "확실한 1티어 최상위" (의대·서울대 최상위·KAIST·POSTECH 영역)
- 13~14 "확실한 1티어" / 11~12 "1티어 가능 + 2티어 안정" / 10 "1티어 도전 + 2티어 안정"
- 그 외 단계도 동일 패턴 (1~10티어 + 비대학)
- FinalTierResult에 confidence·primaryTier·safetyTier·confidenceLabel 4개 필드 추가
- system prompt §13: "어머니에게 환상 ✗, 현실 ✓"

**Self-test 인프라 + 사용자 검증 도입**
- scripts/eval-jaeho.ts (신규): Anthropic API 직접 호출 (prod·DB·사용자 IP 안 거침). jaeho 사주 3회 응답 자동 검증 (메타 노출·baseline 준수·시기 라벨 일관)
- scripts/eval-calibration.ts (신규): 학운 강/중/약 임의 사주 자동 calibration
- tsx devDependency 추가

**실제 사주 calibration (남편 POSTECH·아내 울산대)**
- 사용자 두 분 사주로 알고리즘 검증 — 개인정보 코드·DB·git에 안 남김 (/tmp 임시 평가 후 삭제)
- 결과: jaeho 12점 매우강 (1~2티어) / 남편 11점 매우강 (1~2티어) / 아내 0점 중하 (5~6티어)
- jaeho v3 100점 결과(SKY 라인)와 알고리즘 1~2티어 정합
- 남편 POSTECH 1티어 알고리즘 정합
- 아내 사주 베이스 5~6 vs 실제 울산대 8~9 격차 — 사주 외 변수(청소년기 환경·동기) 해석. "중학교부터 공부 안 함" 패턴은 사주 잠재력이 실현 안 된 케이스로 명리적 해석 가능.

**외부 학술·전문가 의견 검증 (2회 진행)**
- 1차: 재극인·일지 12운성·정인격+비겁·KCI 학업운 점수 모델 검색 → 우리 9가지 보강 중 8가지 명리 합의 부합, 합·삼합만 학파 논쟁
- 2차: 명리 입시 예측 모델·입시 컨설팅 분류·KCI 학업운 점수화 검색 → 명리 점수화 학술 모델 0건 (3대 학자도 표준화 ✗), 입시 컨설팅 "도전/적정/안정" 표준 분류, ML 학업성취 예측 학술 논문 있음 (사주 무관)
- 결론: 우리 모델은 명리 학설 + 입시 컨설팅 표준의 합리적 합성. 권위 검증 데이터 없으므로 mom test 10명 calibration 필요.

**메모리 저장 (협업 패턴)**
- feedback_self_test_before_handoff.md (신규): 검증 책임 LLM이 가져가기, 사용자에게 던지지 말기
- feedback_honest_college_range.md 갱신: 부모 환경 변수 ±2 매핑 학교 티어 기반 정밀화

### 실패한 시도

- 알고리즘 가중치 조정 시도 1차 (관성 +2 → +3, 인성 0+관성 보완) 후 self-test 검증 — jaeho 11점 (의도 1~2티어 ✓) but POSTECH 케이스 calibration 안 했음. 사용자 실제 사주 calibration 후 두 번째 조정 사이클 필요.
- 임의 사주 calibration 시도 — 우연히 강 사주만 잡혀서 진짜 학운 약 케이스 검증 한계. 실제 약 사주 데이터 필요.

### 다음 액션

1. Eugene mom test 10명 진행 — 실제 진학·학업 결과 vs 우리 알고리즘 점수 분포 비교. 분포 편향 (양수 쏠림) 검증.
2. mom test 결과로 cutoff·safety tier 폭 미세 조정 (reach safety +2, 단계 폭 비대칭 등 보류 항목)
