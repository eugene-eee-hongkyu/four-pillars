...
## 마지막 실행: 2026-08-13 19:00
## 마지막 업데이트: 2026-08-13 19:00
## 현재 모드: bypassPermissions

### 현재 집중

- **결제 → 리포트 발송 파이프라인 + 셀프복구 어뷰징 방지까지 완성·프로덕션 검증됨.** 결제 시 요약본 PDF 즉시(메일1) + 백그라운드 크론이 14영역 상세본 PDF 생성·발송(메일2). 셀프 복구(내 리포트·재발송·미수신 배너)에 **재발송 3회 제한(요약·상세 각각)** + 이메일 변경/발송 분리 추가. 어드민 id/pw 인증 분리.
...
- **셀프 복구 + 어뷰징 방지**: `/reports`(내 리포트 — 세션 기준 조회·요약/상세 재발송·이메일 변경) + 홈 미수신 배너 + 헤더 '내 리포트'. `/api/reports`(GET 목록·POST 재발송/setEmail, uuid 방어). **재발송은 발송 성공 후 요약·상세 각 3회 제한**(summary/detail_resend_count, 서버 429 강제), 최초 발송·이메일 변경은 무제한. '이메일 바꾸기'=주소만 저장, 발송은 따로
