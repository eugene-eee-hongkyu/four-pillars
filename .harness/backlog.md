...
## 2026-05-25: DIRECTION_SYSTEM_v3 구현 명세 작성

- **백로그 이유**: DIRECTION_SYSTEM_v3_RESEARCH.md의 개념 연구 완료 후, 데이터 스키마·쿼리 로직을 구체화하는 다음 단계. Eugene의 복수 카테고리 처리 방식(A vs B)을 먼저 결정해야 구현 방향이 정해짐
- **할 것**: DIRECTION_SYSTEM_v3.md 파일 작성 — 학운(단일 티어)과 방향성(10개 카테고리) 매핑 스키마, 메타 라벨·발현 조건 규칙, 쿼리 예시 명시
- **필요한 것**: 사용자 선택 (아래 A/B)
- **이전 검토**: RESEARCH.md에서 명리학 5가지·사회과학 5가지 우려 통합, 정편배합·격국 우선·RIASEC 매핑·발현조건·메타라벨 기둥 확정

---

### Eugene 복수 카테고리 처리 결정

**A) 주 카테고리 1개로 단순화** — 한 명당 최우선 카테고리만 (예: Eugene은 "창업자립형")  
**B) 복수 카테고리 동시 기록** — 사용자가 여러 카테고리에 점수 (예: 과학·공학기술 + 경영·사업상경 동시)

**추천: B (복수 카테고리)**

**이유**: 학운도 하나의 단일 수치인 것처럼, 방향성도 "사용자-카테고리" 다대다 매핑이 가장 깔끔한 데이터 구조. Eugene처럼 실제 특성을 의도적으로 손실하지 않고, UI에서 필요하면 "주 카테고리" 필터링/우정렬은 쿼리에서 처리하면 됨.

어떻게 하실래요? A나 B 선택해주시면 DIRECTION_SYSTEM_v3.md 구현명세 진행하겠습니다.