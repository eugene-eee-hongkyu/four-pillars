...
## 2026-09-18: 약관·개인정보처리방침 잔여 정비 (문구)

- **백로그 이유**: 사용자가 환불 분쟁 직결 3종(결제 전 동의·다운로드 확인 페이지·환급 3영업일)만 먼저 처리하기로 결정. 나머지는 문구 작업이라 판매 개시 전에만 끝내면 됨
- **할 것**:
  1. 개인정보처리방침 — 위탁 목록에 Resend(이메일 발송) 추가, Supabase 위탁 범위에 '리포트 PDF 파일 보관' 추가, 보유기간에 '리포트 PDF: 이용기간 만료 후 1년(기본 결제일+2년)' 추가, 결제 시 수집항목을 실제(이메일만)로 정정, 구글 로그인 노출 여부 확인 후 반영, 위탁 업체 전원 해외 → 국외 이전 고지(국가·항목·보유기간) 추가
  2. 약관 — 회원 탈퇴 방법(이메일 요청) 명시, 제공 시점(요약본 결제 직후·상세본 당일 중) 명시, 제7조 전면 면책에 '회사의 고의 또는 중대한 과실 제외' 단서
  3. 통신판매업 신고번호 placeholder 채우기(신고 완료 후)
- **필요한 것**: 국외 이전 고지는 법률 확인 권장. 부가세 표기 필요 여부는 세무 확인(사업자가 해외 법인 영업소)
- **이전 검토**: 2026-09-18 약관·환불정책·방침 전문을 코드와 대조. 법정대리인 동의 체크박스는 family-input에 실재(OK). 회원 탈퇴 기능은 코드에 없음. 구글 로그인은 useAuth에 provider 코드 존재. 결제 화면 자체 동의는 이번에 추가됨
- **관련 파일**: [eduluck/app/legal/privacy.tsx](../eduluck/app/legal/privacy.tsx), [eduluck/app/legal/terms.tsx](../eduluck/app/legal/terms.tsx), [eduluck/lib/legal/business-info.ts](../eduluck/lib/legal/business-info.ts)
...
## 2026-06-15: A안 코드 커밋 및 후속 테스트

- **백로그 이유**: 코드 수정 완료 후 커밋 대기 중; 설계 불일치 사항 확인 필요; 배포 후 실제 동작 검증 필요
- **할 것**: 
  1. interpret-premium.tsx, HagunSignerBreakdown.tsx 변경사항 커밋
  2. 아빠 절반 가중치 로직 (주석 vs +1 구현) 불일치 확인 및 통일
  3. A안 배포 후 14섹션 LLM 출력 + mom 계정으로 동작 테스트
- **필요한 것**: 사용자 커밋 지시; 아빠 보정 로직 설계 의도 확인; 배포 완료
- **이전 검토**: typecheck 통과, A안(학운·아빠 부모보정 통일) 코드 수정 완료, decision.md에 3가지 대안 기록

## 2026-06-20: 배포 후 14섹션 LLM 출력 점검

- **백로그 이유**: 배포 완료 후에 검증 가능한 작업
- **할 것**: 배포 후 14섹션 LLM 출력이 정상 범위인지 점검
- **필요한 것**: 메인 배포 완료
- **이전 검토**: 홈 재진단 버튼은 캐시 제거 후 정확성 우선 정책 적용 결정

## 2026-06-20: mom test 친구 배포 + 인터뷰 4문항

- **백로그 이유**: 사용자 피드백 수집 단계 (배포 후)
- **할 것**: mom test 친구 대상 배포, 4문항 인터뷰 진행, /admin/feedback 조회
- **필요한 것**: mom test 배포 환경 세팅
- **이전 검토**: (없음)

## 2026-06-20: 통신판매업 신고 + 포트원 PG 가맹점 심사

- **백로그 이유**: 비즈니스 운영 작업 (기술 개발 외)
...
## 2026-08-14: Vercel 배포 후 비회원 결제 경로 E2E 검증

- **백로그 이유**: interpret-premium.tsx 배포 완료 후 시크릿 창에서 실제로 "로그인 없이 결제 도달"이 동작하는지 토스 검수 시나리오대로 검증 필요. Part 1 완료 → 비회원 결제 버튼 노출 → 토스 결제 페이지 진입 전체 흐름 확인
- **할 것**: Vercel 배포 완료 대기 후 시크릿 창에서 Part 1(비회원용 설문) 완료 → 결제 버튼 진입 → 토스 페이지 도달 E2E 테스트. sessionId 기반 checkout이 Part 2 진입 없이 정상 동작하는지 확인
- **필요한 것**: Vercel 프로덕션 배포, 토스 테스트 환경 접근
- **이전 검토**: Part 1 직후 비회원 결제 진입로 구현·커밋 완료. Part 2 로그인 벽은 회원 수확 퍼널 보존 목적으로 유지 결정

## 2026-09-18: 토스 검수 재진행 + 라이브 키 교체

- **백로그 이유**: 리포트 배포 방식 변경(이메일 첨부 → 보관소 + 다운로드 링크)과 결제 시 동의 서버 기록 추가로 인해 토스 검수 재진행 필요. 샌드박스에서 라이브로 전환하려면 재검수 통과 필수
- **할 것**: 
  1. 토스 측에 결제 흐름 변경 사항 안내 (동의 기록 로직 추가)
  2. 토스 재검수 신청 및 통과 대기
  3. 통과 후 TOSS_CLIENT_KEY를 라이브 API 키로 교체
  4. 프로덕션 환경에서 엔드투엔드 결제 테스트
- **필요한 것**: 토스 담당자 연락, 라이브 API 키 발급
- **이전 검토**: 2026-09-18 세션에서 결제 시 동의 기록 로직 추가됨. 현재 환경변수에 TOSS_CLIENT_KEY=샌드박스 값

## 2026-09-18: 어드민 '1년 연장' 버튼 검증

- **백로그 이유**: 리포트 2년 보관 정책으로 1년 단위 연장 기능 구현됨. 어드민이 사용자 보관 기한을 연장할 수 있어야 하는데 검증 미완료
- **할 것**: 
  1. 어드민 패널에서 사용자 리포트 선택 후 '1년 연장' 버튼 동작 테스트 (기한이 정확히 1년 앞으로 이동하는지)
  2. 데이터베이스에 연장 기록이 정상 저장되는지 확인
  3. 연장 후 사용자 마이페이지의 다운로드 기한 표시가 업데이트되는지 검증
- **필요한 것**: 어드민 계정, 테스트용 사용자 리포트 데이터
- **이전 검토**: 2026-09-18 세션에서 리포트 2년 보관 및 1년 연장 로직 구현. 어드민 UI 기능은 준비되었으나 테스트 미완료