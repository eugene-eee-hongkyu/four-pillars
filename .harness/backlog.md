...
## 2026-04-29: CLI 프롬프트 테스트 스크립트 작성

- **백로그 이유**: 대상 프롬프트 종류와 출력 형식이 아직 미결정
- **할 것**: `test-prompt.ts` CLI 스크립트 작성 — Anthropic API 호출로 프롬프트 변경 전후 결과를 비교할 수 있게
- **필요한 것**: 테스트할 프롬프트 종류 확정 (사주 해석, 채팅 등), 출력 형식 결정 (diff, JSON, 점수 등)
- **이전 검토**: 어드민 UI(A) vs Workbench(B) vs CLI 스크립트(C) 비교 완료 → C+B 조합 채택. 화면 폭 통일 작업은 완료됨 (640px/100%)