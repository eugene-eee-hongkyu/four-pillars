...
## 2026-09-18: 다운로드는 2단계(링크=확인 페이지, 버튼=실제 다운로드) + 결제 전 환불 제한 동의를 서버에 기록

- **선택**: `/api/download` GET은 확인 페이지만 보여주고 아무것도 기록하지 않음. 그 페이지의 버튼(POST)을 눌러야 파일 전달 + `first_downloaded_at` 기록. 결제 화면에 필수 동의 체크를 두고, 서버(`/api/payments/order`)가 동의 없으면 주문을 만들지 않으며 `refund_policy_agreed_at`에 시각 기록.
- **대안 검토**:
  - **링크 클릭 즉시 다운로드(1단계)**: 클릭 한 번 줄지만, 메일 보안 프로그램이 링크를 자동 방문하면 고객이 안 눌렀는데 '다운로드함'으로 기록 → 환불 부당 거절. 탈락
  - **User-Agent로 봇 판별**: 보안 게이트웨이는 일반 브라우저 UA를 쓰는 경우가 많아 신뢰 불가. 탈락
  - **동의를 화면에서만 확인**: 우회 가능 + 분쟁 시 증거 없음 → 서버 강제·시각 기록 채택
- **선택 이유**: 환불정책의 기준이 '내려받은 시점'이라 그 기록의 정확성이 곧 분쟁 승패. 전자상거래법 §17 ②5호·⑥상 디지털 콘텐츠 환불 제한은 사전 고지·체험 수단이 있어야 유효 → 결제 전 동의 + 확인 페이지 재고지 + 무료 진단을 체험으로 명시
- **영향 범위**: `api/download.ts`, `api/payments/order.ts`, `app/(flow)/checkout.tsx`, `app/legal/refund.tsx`·`terms.tsx`, 마이그레이션 `20260918020000`. `/reports`의 '바로 내려받기'도 확인 페이지를 거침(클릭 1회 추가)
- **되돌리는 방법**: download GET에서 바로 파일 전달로 복귀(POST 로직을 GET으로). 동의 강제는 order API의 검사 분기 제거

## 2026-09-18: 리포트 제공 = 이메일 첨부 → Supabase Storage 보관 + 1년 다운로드 링크

- **선택**: PDF를 Supabase Storage 비공개 버킷에 보관하고 메일·구매 내역에는 토큰 링크만 제공. 이용기간 결제일+1년(어드민 연장 가능), 만료 후 1년 더 보관 뒤 크론이 파일 삭제(주문 기록은 보존). 파일은 다운로드 API가 직접 전달. 재발송 3회 제한은 폐지하고 주문당 60초 간격만.
- **대안 검토**:
  - **보관소 — Vercel Blob / S3**: 새 업체·새 비밀키 필요. Supabase는 이미 사용 중이고 service_role로만 열리는 비공개 버킷이라 추가 자격증명 0 → 채택
  - **전달 — Storage 서명 URL로 302**: 함수 대역폭을 안 쓰지만 한글 파일명이 이중 인코딩돼 깨짐(실측) → 직접 전달 채택(PDF 약 150-300KB), 4MB 초과 시에만 서명 URL 우회
  - **만료 후 즉시 삭제 / 영구 보관**: 사용자 결정으로 '만료 후 1년 추가 보관, 2년 뒤 삭제'. 용량 부담 미미 + 분쟁 시 복구 여지
  - **재발송 제한 유지(3회)**: 링크 재전송은 비용 0이라 불필요(사용자 결정). 단 메일 발송 한도 소진 방지를 위해 60초 간격은 둠(어드민 예외)
- **선택 이유**: PG 측의 '이용기간 1년' 요구를 실제로 강제할 수 있는 구조가 첨부 방식으로는 불가능. 링크 방식은 재발송 시 재생성 비용도 없앰
- **영향 범위**: `lib/payments/report-storage.ts`(신설)·`fulfill.ts`·`fulfill-detail.ts`, `lib/email/send-report.ts`, `api/download.ts`(신설)·`api/reports.ts`·`api/admin/payments.ts`·`api/cron/purge-expired-pdfs.ts`(신설), `vercel.json`(크론·함수 설정), `app/reports.tsx`·`app/admin/payments.tsx`, 약관 제6조·환불정책·상품 설명, 마이그레이션 `20260918000000`·`20260918010000`
- **되돌리는 방법**: send-report에 attachments 복원 + fulfill에서 storeReportPdf 대신 첨부 전달. 버킷·컬럼은 남겨도 무해. 약관 제6조 4-6항·상품 설명 문구도 함께 되돌려야 함

## 2026-08-14: 토스 검수 대응 — 비회원 결제 도달 경로 추가

- **선택**: Part 1 완료 직후 비회원 전용 결제 진입로 추가 (A 방안)
- **대안 검토**: 
  - Part 2 로그인 벽을 완전히 제거 → 회원 수확 퍼널 손상
  - 비회원 상태로 Part 2 진입 허용 → 전체 UX 구조 변경 필요
- **선택 이유**: 토스 검수자가 로그인 없이 결제까지 테스트 가능하면서, 기존 회원 수확 흐름(Part 2 로그인)은 유지. checkout은 sessionId만으로 작동하므로 Part 2 열람 불필요
- **영향 범위**: `interpret-premium.tsx` 파일에 결제 진입로 조건 추가
- **되돌리는 방법**: 비회원 결제 진입 조건문 제거 후 원래 Part 2 로그인 강제 로직으로 복원
