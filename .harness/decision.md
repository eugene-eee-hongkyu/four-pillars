...
## 2026-05-18: 사주톡 vs 학운 프로젝트 만세력 적용 방식

- **선택**: B안 (사주톡 변경 없음, 학운 프로젝트에만 정확한 만세력 적용)
- **대안 검토**: 
  - A안: 사주톡도 함께 정확한 만세력으로 변경 (정확도 향상, 통일된 데이터 기반)
  - B안: 사주톡은 현재 상태 유지, 학운은 정확한 만세력 적용 (변경 최소화, 진행 중 테스트 보호)
- **선택 이유**: 사주톡 현재 10명 테스트 진행 중 → 변경 최소화로 결과 일관성 유지. 테스트 완료 후 정확한 피드백 받고 2차 배포 시 판단. 학운은 신규 프로젝트이므로 처음부터 정확한 만세력으로 차별화.
- **영향 범위**: 사주톡 테스트 데이터 일관성 / 학운 초기 구현 (만세력 조회 API)
- **되돌리는 방법**: 테스트 완료 후 사주톡도 정확한 만세력으로 변경하려면, 기존 API 호출 로직을 새 로직으로 교체하고 재배포
