...
## Session 2026-05-24 07:19 — 시그너 가중치 calibration loop 90회 자동 실행 + 사회 분포 cutoff 시스템

### 작업 요약
- **자기비판·방향 전환**: "L2 신살 ad-hoc 위험" 명분 폐기. 사용자 비판 ("기존 명리학이 1티어를 못 잡으면 그건 쓰레기") 정직 인정. 9 sample을 ground truth로 fitting하는 것이 본질, 명리 학파 정통성 안주는 결정 회피였음을 인정.
- **30단계 내부 티어 시스템 설계**: 10티어 (외부 노출) × 3 (엄청 강·강·약강) = 30단계. 한국 사회 분포 기준 cutoff (1티어 5%·2티어 7%·…·10티어 10%) 자동 산출.
- **100만 random 사주 시뮬 + 1만·10만·100만 cutoff 안정성 검증**: 핵심 cutoff (1-3=55, 1-2=57, 1-1=61) 1만·10만·100만 ±1점 일치 확인. **1만 시뮬이 calibration loop 정밀도에 충분** 입증.
- **9 sample 목표 매트릭스 정정**: 세형(연대 의예)이 1-1 (의예 최상위) 아님. 1티어 5명 모두 1-2 (강) 영역 = 점수 57-62 동등. 사용자 정정 반영.
- **윤수 "최상위 회고" 정정**: 1티어 5명 학교 성적 대부분 동등. 사회적 성공(부사장)은 학운과 별개 차원. 10-yoonsoo.md + HAGUN_LOOCV.md cross-ref 표 정정.
- **V1 30회 calibration loop 자동 실행** ([scripts/run-calibration-30.ts](../eduluck/scripts/run-calibration-30.ts), 1만 sample × 30 시나리오): V1 Top 3 = Loop 27 (wgap 106), Loop 6 (108), Loop 18 (110).
- **V2 60회 calibration loop 자동 실행** ([scripts/run-calibration-v2.ts](../eduluck/scripts/run-calibration-v2.ts), Part A 30 + Part C 30): V2 Top 3 = Loop 58 (wgap 89), Loop 90 (89), Loop 88 (96). V1 대비 17점 개선.
- **새 detector 확장**: count (insung·gwansung·jaesung·hakdang·munchang·cheoneul 등 12종), combo (scholar·jarip·gwaninStrong·yanginScholar·allScholar·youngshik 7종), timing (early_6_15·exam_16_22 3종), threshold (insung_1+/4+·gui_2+/3+ 5종).
- **자동 보고서**: docs/CALIBRATION_LOOPS.md (V1 30회) + docs/CALIBRATION_LOOPS_V2.md (V2 60회) 자동 생성. 다음 세션 안 시도 방향 가이드 포함.

### 실패한 시도
- **wgap 89 한계 도달 (90회 시뮬 모두)**: 영진(상관격 4티어, gap 18-19), 정환(정재격 1티어, gap 12-16), 두흥(외부 변수, gap 14-18) 격차 잔존. 사주만으로 fit 한계 있음.
- "ad-hoc 위험" 명분으로 결정 회피 (사용자 명확한 비판으로 정정).

### 다음 액션
- V3 30-60회 calibration: 개별 sample fine-tuning (Top 3 weight 기준 + 영진·정환·두흥 강화) + 안 시도된 8가지 방향 (격국별 페널티 차등·세운 가중·충형해파 단계별·신살 카운트 단계별·Bayes 최적화 등)
- 또는 V2 Top 3 (Loop 58·90) 시스템 적용 + 외부 100명 단계 진입
- 결정 보류 시 사주만으로 fit 한계 인정 + 외부 변수 (환경·노력·SES) 별도 모듈 도입 검토
...
**문서·기록**
- [eduluck/docs/HAGUN_REFACTOR_ANALYSIS.md](../eduluck/docs/scoring/HAGUN_REFACTOR_ANALYSIS.md) 신규 — 22→14 리팩토링 분석
...
**산출물**
- [eduluck/docs/HAGUN_REFACTOR_ANALYSIS.md](../eduluck/docs/scoring/HAGUN_REFACTOR_ANALYSIS.md) — 전체 분석 + 3-Layer 권장안 저장
