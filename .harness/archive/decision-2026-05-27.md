...
## 2026-05-27: V13 영진(07) fit — narrow trigger 콤보 채택

- **선택**: A (영진 specific 좁은 fingerprint 콤보 +31 raw) + B (외부변수 안내 prompt 분기) 조합
- **대안 검토**:
  - **A (채택)**: `combo_sanggwanArtsMediaConvergence` — 상관격 + 학자귀인 0 + 청소년 학자대운 0 + 화국 삼합 + 도화살 + 화개살 6 조건 AND. 영진만 매칭 보장 (11 sample false positive 0)
  - **B (병행)**: NON_SCHOLAR_GYEOKGUK + isScholar=false sample LLM 톤 보강 (점수 변동 ✗, prompt만)
  - **C (거부)**: 자가 입력 (노력형/평균/기복형 선택) — mom test UX 부담 + 자가 답변 신뢰도 ✗
  - **D (거부)**: 영진 fit 포기, expected weight 0.5 인정 — 사용자 본인 진단 9-10티어 표시 시 신뢰 직격
  - **B안 수정판 (시도 후 롤백)**: baseScore 18→33 + 페널티 약화 + 콤보 강화. 영진 fit 됐으나 와이프·승희·박진우 등 다른 비학자 격국 sample 동시 위로 이동
- **선택 이유**:
  1. A 는 영진만 매칭 보장 — overfitting 명시 인정하되 mom test 사후 검증 가능
  2. B 는 score 변동 없이 영진·와이프·박진우·재원 등 비학자 격국 + isScholar=false sample LLM 톤 정직+희망으로 보강
  3. C 는 mom test 진입 비용. 도입 시 외부변수 모듈 backlog 와 묶어 별도 세션 적합
  4. D 는 영진 본인 사주 진단 시 사용자 신뢰 손상
- **영향 범위**:
  - `lib/prompts/hagun-tier.ts` (콤보 +31), `lib/prompts/interpret-premium-shared.ts` (NON_SCHOLAR_GYEOKGUK + 외부변수 안내), `lib/flow/context.tsx` (PROMPT_VERSION v5.13)
  - `_private/calibration-samples/data.ts` (영진 expected 36.9), `scripts/eval-youngjin-trigger.ts` 신규
- **되돌리는 방법**: V12 (commit `cb2df11`) 로 hagun-tier 부분 revert + PROMPT_VERSION 원복

---

## 2026-05-27: v4 legacy interpret-premium + 부모학력 입력 일괄 단절

- **선택**: v4 endpoint·prompt·DB 데이터·부모학력 모듈 전부 삭제 (Phase A + B 일괄). 옛 사용자 데이터 보존 ✗.
- **대안 검토**:
  - **A (채택)**: v4 endpoint·prompt·context 필드·DB column·UI route 모두 제거 + DB row 삭제 + CHECK constraint 제거. legacy 흔적 0.
  - **B**: v4 endpoint 만 비활성 (file 유지), DB 보존, context 필드 deprecated 표시. 캐시 무효만으로 자연 마이그레이션
  - **C**: v4 도 v5.13 prompt 로 redirect. v4 endpoint 살리되 내용만 새 prompt 사용
- **선택 이유**:
  - 사용자 명시: "옛 결제 고객도 테스트였기 때문에 DB 도 삭제하고 관리하지 않아도 된다"
  - v4 단절 효과 명백 (49KB v4 prompt + endpoint 코드 dead). Phase H 이후 부모학력 가중치 0 — UI dead. 일괄 정리 = 추후 유지 비용 ↓
  - 옛 결제 = 모두 테스트, prod 사용자 실데이터 ✗
- **영향 범위**:
  - 코드 1764줄 삭제 (v4: endpoint 2개 + prompt 49KB + scripts 7개)
  - 코드 ~150줄 추가 삭제 (부모학력: university-tier.ts·resolveParentTier·ParentEducation type·UI route·API endpoint)
  - DB migration 2건 (kind='premium' rows 삭제 + interpretations_kind_check 제거 + subjects.education_json column drop)
  - PROMPT_VERSION v5.13 (캐시 자동 무효 → v5 part1/part2/deep 만 사용)
- **되돌리는 방법**:
  - 코드: 5개 commit revert (`e1c9e1b`·`a20b5a5`·`3021218`·`13b00f9`·`6c8cc13`)
  - DB: migration 역순 SQL (CHECK 재추가 + education_json column 추가). 단 옛 데이터 복원 불가 (삭제됨)
