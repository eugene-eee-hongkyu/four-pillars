...
## 2026-05-24: 정환 sample weight 1 → 0.5 (외부 변수 인정)

- **선택**: 정환 sample weight 0.5 (두흥·영진과 동일 — 외부 의지·노력 sample 분류)
- **대안 검토**:
  - A (현 채택): weight 0.5. calibration metric에서 정환 영향 ↓. 시스템이 정환 못 잡아도 다른 sample 우선
  - B: weight 1 유지 + 정관격/정재격 detector 추가 sweep. 매번 정환 gap 6+ — 시스템 한계
  - C: 정환 sample 제거. ground truth 손실
- **선택 이유**:
  - 정환은 정재격 + 관성 강 + 관인상생 + 청소년 관성 대운 — 사주 학자 신호 있으나 SKY 도달은 외부 환경/노력
  - 두흥(비대학/외부변수)·영진(연예인 의지)과 동일 패턴 — "사주 본질로는 SKY 못 잡지만 실제 도달" 외부 변수 sample
  - calibration 정확도 ↑ + 학자 본질 약한 sample도 ground truth로 유지
- **영향 범위**: 모든 calibration script (run-calibration-30/v2/v3/v4/v5/v6) SAMPLE_TARGETS
- **되돌리는 방법**: 정환 weight 1로 복원 + 정관 시너지 콤보 강화 sweep 재실행

---

## 2026-05-24: V10 Loop 523 (비견격 학자형 콤보) 채택 — 재호 1-3 도달

- **선택**: V10 Loop 523 — combo_bigyeonGwansung·Gwangwi·Munchang 각 +6 (균등)
- **대안 검토**:
  - V10 Loop 503: combo_bigyeonGwansung +20 단독. totalGap 21.5 동등, 재호 1-3
  - V10 Loop 510/530: 비견 콤보 +12·+13. 재호 1-1까지 over-shoot
  - V8 Loop 335 채택 (재호 2-2): V10 못 거치고 종료. 사용자 요구 "재호 1-3 이상" 미달
- **선택 이유**:
  - 학파 인용 3개 (자평진전·적천수·삼명통회) 분산형 — 단일 detector 의존 ↓
  - 재호 1-3 정확 도달 (norm 89.4) — 사용자 요구 정합
  - 다른 sample (홍규·세형·윤수·상수·두흥·승희·와이프) over-shoot ✗ — 균형 유지
- **영향 범위**: 다음 prod 반영 시 hagun-tier.ts에 비견격 학자형 콤보 4개 detector + Loop 523 weight 통합
- **되돌리는 방법**: V8 Loop 335 weight로 복원 (재호 2-2 영역). 또는 V10 Loop 503 (단일 강 weight)

---

## 2026-05-24: 음력→양력 변환 — lunar-typescript 활용 (자체 알고리즘 ✗)

- **선택**: `lunar-typescript` 라이브러리 활용 (이미 설치, 절기 계산 용도로 사용 중)
- **대안 검토**:
  - A (현 채택): lunar-typescript `Lunar.fromYmd().getSolar()`. 천문 알고리즘 기반 (Meeus). KASI 공식과 6초 오차
  - B: 자체 알고리즘 구현. Meeus Astronomical Algorithms + ELP-2000(달) + VSOP87(태양). 구현 복잡, 6개월+
  - C: KASI 공식 API 호출. 외부 의존 + 캐싱 필요
  - D: lookup table (1900-2100). 100년 데이터 + ~1MB. 정밀도 확보
- **선택 이유**:
  - 이미 의존성 있음 (절기 시각, [solar-terms.ts:17](../eduluck/lib/manse/solar-terms.ts#L17))
  - 천문 알고리즘 기반이라 알고리즘 정확성 검증 가능
  - 1줄 코드: `Lunar.fromYmd(y, m, d).getSolar()`
  - CST(UTC+8) → KST 보정 패턴 이미 검증됨
- **영향 범위**: 사용자 음력 입력 처리 시점. 향후 UI에 음력/양력 토글 추가 가능
- **되돌리는 방법**: KASI API로 교체. 또는 lunar-typescript 제거 후 자체 구현 (시간↑)
