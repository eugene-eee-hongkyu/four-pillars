        ...
        <View className="px-container-padding gap-3">
          ...
          <View className="p-card-padding rounded-md border border-outline-warm bg-secondary-container/30 mt-2">
            ...
            <Text className="font-body text-body-md text-text-pri leading-relaxed">
              PDF를 내려받기 *전*: 결제일로부터 7일 내 100% 환불{'\n'}
              PDF를 내려받은 *후*: 환불 불가 (디지털 콘텐츠 특성){'\n'}
              다운로드 이용기간: 결제일로부터 1년 (기간 만료는 환불 사유 아님)
            </Text>
          </View>
          ...
          <Section title="1. 청약철회 가능 조건">
            ...
            {'(1) 결제일로부터 7일 이내\n'}
            {'(2) 리포트(요약본·상세본)를 아직 한 번도 내려받지 않은 상태\n'}
            {'   - "내려받기"란 이메일 또는 리포트 구매 내역의 링크로 열리는 다운로드 확인 화면에서 "PDF 내려받기" 버튼을 눌러 파일을 받은 것을 말합니다.\n'}
            {'   - 링크를 열어 확인 화면만 본 것은 내려받기에 해당하지 않습니다.\n'}
            {'   - 회사는 최초 내려받기 시각을 기록하며, 이를 기준으로 판단합니다.'}
          </Section>

          <Section title="2. 청약철회 제한 조건 (전자상거래법 §17 ② 5호)">
            {'다음 시점부터 청약철회가 제한됩니다:\n\n'}
            {'다운로드 확인 화면에서 "PDF 내려받기" 버튼을 눌러 리포트(요약본·상세본 중 어느 하나라도)를 내려받은 시점\n\n'}
            {'※ 본 서비스의 PDF 진단 결과는 1회 열람으로 핵심 가치(개인 맞춤 사주 해석)가 즉시 소비되는 디지털 콘텐츠입니다. 「전자상거래 등에서의 소비자보호에 관한 법률」 제17조 제2항 제5호에 따라 청약철회를 제한합니다.\n\n'}
            {'회사는 결제 전 무료 진단을 제공하여, 이용자가 결제에 앞서 콘텐츠의 내용과 품질을 미리 확인할 수 있도록 하고 있습니다.\n\n'}
            {'결제 화면에서 본 제한 사항과 다운로드 이용기간에 대한 필수 동의를 받으며(동의 시각 기록), 다운로드 확인 화면에서도 내려받기 전에 다시 안내합니다.'}
          </Section>

          <Section title="3. 사업자 귀책사유로 인한 환불 (100% 환불 의무)">
            {'다음의 경우 회사는 100% 환불을 보장합니다:\n\n'}
            {'(1) 시스템 오류로 다운로드 링크 안내 메일이 정상 발송되지 않고, 리포트 구매 내역에서도 내려받을 수 없는 경우\n'}
            {'(2) 내려받은 PDF 파일이 손상되어 열람이 불가능한 경우\n'}
            {'(3) 결제 후 7일이 경과해도 리포트(요약본·상세본)를 내려받을 수 있는 상태가 되지 않은 경우\n'}
            {'(4) 이용기간(결제일로부터 1년) 이내임에도 회사의 시스템 문제로 다운로드가 불가능하고, 회사가 이를 복구하지 못한 경우\n'}
            {'(5) 진단 내용에 명백한 시스템 오류로 인한 결과가 포함된 경우\n'}
            {'(6) 이용자가 입력한 정보와 명백히 다른 내용이 제공된 경우'}
          </Section>

          <Section title="4. 다운로드 이용기간(1년)과 환불">
            {'(1) 리포트는 결제일로부터 1년간 다운로드 링크 및 리포트 구매 내역을 통해 내려받을 수 있습니다 (이용약관 제6조).\n'}
            {'(2) 이용기간이 지나 링크가 만료되어 내려받을 수 없게 된 것은 환불 사유에 해당하지 않습니다. 이용기간 내에 파일을 내려받아 보관하시기 바랍니다.\n'}
            {'(3) 이용기간 내에 한 번도 내려받지 않은 경우에도, 청약철회 기간(결제일로부터 7일)이 지난 뒤에는 환불되지 않습니다.'}
          </Section>

          <Section title="5. 무료 진단 서비스 (해당 없음)">
          ...
          <Section title="6. 환불 신청 방법">
            ...
            {'   - 결제 영수증 또는 거래 번호\n'}
            {'   - 환불 사유\n'}
            {'(3) 처리 기간: 환불 요건에 해당하는 경우 신청 접수일로부터 3영업일 이내에 결제 대금을 환급합니다 (전자상거래법 제18조). 카드사·결제기관 사정에 따라 실제 취소 반영까지는 추가로 시일이 걸릴 수 있습니다.\n'}
            {'(4) 환불 수단: 원 결제 수단으로 환불 (카드 승인 취소 또는 계좌 입금)'}
          </Section>

          <Section title="7. 부분 환불">
            {'본 서비스는 단건 PDF 결제 모델로, 부분 환불은 적용되지 않습니다. 결제 단위로 전액 환불 또는 환불 불가가 결정됩니다.'}
          </Section>

          <Section title="8. 분쟁 해결">
