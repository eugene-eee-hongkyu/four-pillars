...
const SAMPLE_TARGETS: SampleTarget[] = [
  { id: '03-self',    nickname: '홍규',  school: 'POSTECH',     target30Index: 2,  targetLabel: '1-2', weight: 2 },
  ...
  { id: '09',         nickname: '두흥',  school: '비대학(외부)',  target30Index: 2,  targetLabel: '1-2(외부)', weight: 0.5 },
...
async function main() {
  ...
  for (const config of SCENARIOS) {
    ...
    console.log(`  완료 ${elapsed.toFixed(1)}s | wgap=${result.totalWeightedGap.toFixed(1)} | mean=${result.distribution.mean.toFixed(1)} stddev=${result.distribution.stddev.toFixed(1)}`);
  ...
  for (let i = 0; i < top3.length; i++) {
    ...
    console.log(`  wgap=${r.result.totalWeightedGap.toFixed(1)}, mean=${r.result.distribution.mean.toFixed(1)}`);
  ...
  console.log(`\n→ docs/CALIBRATION_LOOPS_V2.md 작성 완료`);
}
...
function writeReport(allResults: ScenarioResult[], top3: ScenarioResult[]) {
  ...
  lines.push('');
  lines.push('## 종합 결과 (weightedGap 오름차순)');
  lines.push('');
  ...
  for (let i = 0; i < top3.length; i++) {
    ...
    lines.push('');
    lines.push('**1티어 영역 cutoff (1-1 ~ 2-3)**:');
    lines.push('');
  ...
  for (const r of allResults.sort((a, b) => a.scenarioId - b.scenarioId)) {
    ...
    lines.push(`- **결과**: weightedGap = ${r.result.totalWeightedGap.toFixed(1)}, mean=${r.result.distribution.mean.toFixed(1)}`);
    lines.push('');
  ...
  writeFileSync('/Users/eugene/Downloads/coding/four-pillars/eduluck/docs/CALIBRATION_LOOPS_V2.md', lines.join('\n'));
}
