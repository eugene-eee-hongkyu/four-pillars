// V6 — V5 시나리오를 absolute cutoff (V4 #195 raw) 기준으로 재측정
//
// V5 발견: 정규화 cancel out 때문에 weight 변경 효과 측정 불가
// V6: V4 #195 raw cutoff fixed → 다른 시나리오 sample 점수 raw로 계산 → 진짜 weight 효과 측정

import { computeManse } from '../lib/manse/engine';
import { SAMPLES } from '../_private/calibration-samples/data';
import { detectAllSigils } from './run-calibration-v3';
import { writeFileSync } from 'fs';
import { SCENARIOS as V5_SCENARIOS, type CalibConfig } from './v5-scenarios';
...
async function main() {
  console.log(`\n=== V6: V5 시나리오를 absolute cutoff (V4 #195 raw) 기준 재측정 ===\n`);
  ...
  for (const config of V5_SCENARIOS) {
  ...
  // markdown 보고서
  const lines: string[] = [];
  lines.push('# V6 Calibration — V5 시나리오 absolute cutoff 재측정');
  lines.push('');
  ...
  lines.push('---');
  lines.push('');
  lines.push('## V5 시나리오 absolute 측정 (전체 30개)');
  lines.push('');
  ...
  lines.push('---');
  lines.push('');
  lines.push('## 🏆 V6 Top 5 (absolute cutoff)');
  lines.push('');
  for (let i = 0; i < top5.length; i++) {
    const r = top5[i];
  ...
  writeFileSync('/Users/eugene/Downloads/coding/four-pillars/eduluck/docs/run/CALIBRATION_LOOPS_V6.md', lines.join('\n'));
  console.log(`\n→ docs/run/CALIBRATION_LOOPS_V6.md 작성 완료`);
}

if (process.argv[1]?.endsWith('run-calibration-v6.ts')) {
