...
// V4는 V3 복사본 (detectAllSigils에 신규 19 detector 이미 포함). SCENARIOS만 v4-scenarios.ts에서 import.
...
import { SCENARIOS } from './v4-scenarios';
...
async function main() {
  ...
  for (const config of SCENARIOS) {
    ...
    console.log(`Loop ${config.id}/240: ${config.name}`);
  ...
  console.log(`\n→ docs/run/CALIBRATION_LOOPS_V4.md 작성 완료`);
}
...
function writeReport(allResults: ScenarioResult[], top3: ScenarioResult[]) {
  ...
  lines.push('---');
  lines.push('');
  lines.push('## 🏆 V4 Top 3 — 30단계 cutoff 통합 비교');
  lines.push('');
  ...
  writeFileSync('/Users/eugene/Downloads/coding/four-pillars/eduluck/docs/run/CALIBRATION_LOOPS_V4.md', lines.join('\n'));
}

// Entry guard: import 시 자동 실행 방지 (eval-detector-profile.ts 등이 detector import 가능)
if (process.argv[1]?.endsWith('run-calibration-v4.ts')) {
