// V3 — 90회 Calibration Loop (a: Top 3 fine-tuning 30회 + b: 안 시도 8방향 60회)
//
// V2 best wgap 89 (Loop 58·90 동률). V3 목표: wgap < 80 도달.
//   Part A (Loops 91-120, 30개): V2 Top 3 (#58·#90·#88) fine-tuning + 영진·정환·두흥 강화 변종
//   Part B (Loops 121-180, 60개): V2 "안 시도 8방향" — 격국 base 차등, 사주 강약 페널티 약화,
//                                    충형해파 시기별, 신살 카운트 multiplier, 상관·정재격 별도 시그너,
//                                    두흥 외부변수 명시 처리, 통합 후보
//
// 신규 출력: Top 3 30단계 cutoff 통합 비교 표 — sample 점수와 각 cutoff 격차를 한눈에
...
import { SCENARIOS } from './v3-scenarios';
...
export function detectAllSigils(m: ManseResult): Record<string, number> {
  ...
  return {
    ...
    // === V5 신규 (학파 ≥ 2 검증, DETECTOR_CANDIDATES_V5.md §2) ===
    gw_gwangwiHakgwan: gwangwiCount >= 1 ? 1 : 0,
    combo_cheonEulHakdang: comboCheonEulHakdang ? 1 : 0,
    ...
    cnt_gwangwiHakgwan: gwangwiCount,
  };
}
...
async function main() {
  const N = 10000;
  console.log(`\n=== V3: 90회 Calibration Loop (a: fine-tuning 30 + b: 안 시도 8방향 60) ===\n`);

  const allResults: Array<{ scenarioId: number; result: ReturnType<typeof evaluateConfig> }> = [];

  for (const config of SCENARIOS) {
    const start = Date.now();
    console.log(`Loop ${config.id}/180: ${config.name}`);
  ...
  console.log(`\n→ docs/run/CALIBRATION_LOOPS_V3.md 작성 완료`);
}
...
function writeReport(allResults: ScenarioResult[], top3: ScenarioResult[]) {
  ...
  lines.push('---');
  lines.push('');
  lines.push('## 🏆 V3 Top 3 — 30단계 cutoff 통합 비교');
  lines.push('');
  ...
  writeFileSync('/Users/eugene/Downloads/coding/four-pillars/eduluck/docs/run/CALIBRATION_LOOPS_V3.md', lines.join('\n'));
}

// Entry guard: import 시 자동 실행 방지 (eval-detector-profile.ts 등이 detector import 가능)
if (process.argv[1]?.endsWith('run-calibration-v3.ts')) {
