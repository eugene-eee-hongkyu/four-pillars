...
9. \`## 9. 현재~앞으로의 흐름 — ${'$'}{한 줄 부제}\` (대운·세운 + 12운성 변화, 6~10문장). **user message [현재 학운 시기] baseline을 그대로 사용** — 학운 강/중/약 시기 라벨 매번 흔들리지 않게 코드 결정값 채택.
    **마지막 단락 (필수)**: user message [조심해야 하는 한 해] baseline을 그대로 사용해 자녀 입시 시기 중 가장 흔들리는 1년 명시 — "○○년(만 ${'$'}{나이}세)이 가장 흔들리는 자리예요. ${'$'}{충/형/자형 시그너 자연 풀이}. ${'$'}{무엇을 조심해야 하는지 1~2개}". 톤은 "사고가 일어난다" 단정 ✗, "흔들리기 쉬워요·집중력 흩어져요·결정 미루지 마세요" 부드럽게.
...
13. \`## 13. 학교 볼게요 — ${'$'}{한 줄 부제}\` ("학교 볼게요" 인사 본문 첫 줄. 학년 분기에 따라 구체 학교명 명시. 권유 톤은 §13 가이드의 "안정·가능·도전" 3구간 표현 사용. 6~8문장).
    **마지막 단락 (필수)**: "본질을 깨우는 가장 효과적 액션" 3가지 카드 — 사주의 강한 시그너를 활용해 학운이 풍부하게 펴지는 환경·시기·접근. 톤: "끌어올린다·SKY 가능" 단정 ✗, **"본질이 ~할 때 가장 빛난다·받쳐진다"** 어조.
    1. **용신 환경 카드**: 용신 오행 기반 환경 1가지 (목→자연·도서관·푸른 공간 / 화→밝은 조명·발표 기회 / 토→안정 루틴·반복 학습 / 금→정리·논리·체계 / 수→독서·사색·조용한 공간)
...
14. \`## 14. 어머니께 한 마디 — ${'$'}{한 줄 부제}\` (자녀 사주 기반으로 어머니가 **지금 이 시점에** 어떻게 서포트해야 하는지 구체 액션, 7~10문장).
    **어머니가 메인 청자**. 자녀의 격국·용신·대운·세운·학년을 근거로 어머니가 지금 일상에서 할 수 있는 구체 액션 권유:
    ...
    - **용신 환경 액션 (1~2개 구체)**: 목→자연·도서관, 화→밝은 공간, 토→안정 루틴, 금→정리·논리, 수→독서·사색
...
**현재는 자녀 사주만 입력받습니다. 어머니가 메인 청자이고 §14는 어머니가 일상에서 할 수 있는 구체 액션 권유로 채워야 합니다.**
...
export function buildInterpretPremiumPrompt(ctx: InterpretPremiumContext): string {
  ...
  const lines = [
    ...
    m
      ...
      : `[어머니 사주] — 미입력 (디폴트). §14는 자녀 사주 기반 어머니 서포트 가이드로 풀이.`,
    ...
    ...(() => {
      ...
      if (!cr.worst) {
        return [
          `[조심해야 하는 한 해 — 학년대 내 위험 시그너 ≥2 없음. §9 마지막 단락 생략하거나 "큰 흔들림은 보이지 않아요" 1줄로 부드럽게 마무리]`,
          ``,
        ];
      }
      const w = cr.worst;
      ...
      return [
        `[조심해야 하는 한 해 — 백엔드 결정성. §9 마지막 단락 baseline. 점수·시그너 이름 본문 노출 ✗, 명리적 근거만 자연 풀이]`,
