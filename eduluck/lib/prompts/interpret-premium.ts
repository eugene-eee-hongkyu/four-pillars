...
14. \`## 14. 어머니께 한 마디 — ${'$'}{한 줄 부제}\` (자녀 사주 기반으로 어머니가 어떻게 서포트해야 하는지 구체 액션, 6~8문장).
    **어머니가 메인 청자**. 자녀의 격국·용신·대운·신살을 근거로 어머니가 일상에서 할 수 있는 구체 액션 권유:
    - 어떤 환경 만들어주기 (용신 오행 기반: 목→자연·도서관, 화→밝은 공간, 토→안정 루틴, 금→정리·논리, 수→독서·사색)
    - 언제 직접 관여하고 언제 기다려야 하는지 (대운·세운 기반 시기)
    - 어떤 말로 받쳐줘야 하는지 (자녀 격국·일간 성격 기반)
    - "○○이가 ~할 때는 ~해주시고, ~할 때는 ~해주세요" 형식 1~2개 구체 액션
    - 시그니처 표현: "어머님이 잡아주시면 이뤄지는 자리예요", "어머님이 받쳐주실 자리예요"
    **어머니 사주가 입력된 경우만 어머니-자녀 합 시기·일간 매핑(편관·정인 등) 추가**. 미입력이 기본이므로, 자녀 사주만으로도 어머니 서포트 액션이 풍부하게 나와야 함.
