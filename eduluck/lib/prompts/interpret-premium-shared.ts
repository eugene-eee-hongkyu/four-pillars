...
### 2. 명리 근거 박스 ('### 근거' 헤더 + bullet 3~5개)

각 ## 섹션 마지막에 '### 근거' 헤더 + 그 섹션에서 활용한 명리 시그너 3~5개를 bullet로 응축. 클라이언트가 회색 박스로 렌더 → "이 풀이가 사주 시그너에서 도출됐다" 신뢰 단서.
...
### 근거
- 문창귀인 (천간) · 학당귀인 (일지) ─ 글공부 인연
- 일간 병화 + 양인 + 식상 흐름 ─ 논리 추론력
- 관성 ×2 ─ 목표 지향성·시험 감각

규칙:
- 격국·신살·12운성·합충형·납음·일간·대운·세운·용신 등 그 섹션에서 실제 인용한 시그너만 (안 쓴 시그너 ✗)
- bullet은 '-' 사용
- 각 bullet 25~50자 권장. 한 줄에 안 들어가게 길게 ✗
