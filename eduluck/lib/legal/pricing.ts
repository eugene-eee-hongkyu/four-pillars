 ...
 *  결제 완료 후 진단 전문 PDF를 보관소에 두고 이메일로 다운로드 링크 발송(이용기간 결제일+1년, 약관 제6조). */
export const PDF_REPORT = {
  price: 30000,
  name: '정밀 학운 리포트 (PDF)',
  description:
    '자녀의 만세력을 기반으로 한 학운 정밀 진단 전문(본질·강점·약점·환경·부모합·양육·친구·흐름·진로·학교 등 14개 영역)을 PDF 리포트로 정리해 드립니다. 결제 후 이메일로 다운로드 링크를 보내드리며, 결제일로부터 1년간 내려받으실 수 있습니다(받으신 파일은 기기에 저장해 계속 보관 가능).',
} as const;
